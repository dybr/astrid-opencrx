...
public class OpencrxRestClient {
	 ...
	 */
	private InputStream processHttpResponse(HttpResponse response)
		...
		if (statusCode != HTTP_OK) {
			...
			if (statusCode == 401)
				error = new ApiAuthenticationException(response.getStatusLine()
						.getReasonPhrase());
			else
	...
	/**
	 * Issue an HTTP GET for the given URL, return the response using given
	 ...
	 * @param opencrxLogin
	 *            login for OpenCRX REST service
	 * @param opencrxPassword
	 *            password for OpenCRX REST service
	 * @return response stream, or null if there was no response
	 * @throws IOException
	 */
	public synchronized InputStream get(String url, String opencrxLogin,
			String opencrxPassword) throws IOException {

		try {
			HttpGet httpGet = new HttpGet(url);
			HttpResponse response = getThreadSafeClient(opencrxLogin,
					opencrxPassword).execute(httpGet);
	...
	public synchronized InputStream post(String url, String data, String login,
			String password) throws IOException {
		try {
			HttpPost httpPost = new HttpPost(url);
			httpPost.addHeader("Content-Type", "application/xml;charset=UTF-8");
			httpPost.setEntity(new StringEntity(data, "UTF-8"));
			HttpResponse response = getThreadSafeClient(login, password)
					.execute(httpPost);
	...
	public synchronized InputStream delete(String url, String login,
			String password) throws IOException {
		try {
			HttpDelete httpDelete = new HttpDelete(url);
			HttpResponse resp = getThreadSafeClient(login, password).execute(
					httpDelete);
	...
	@SuppressWarnings("nls")
	public synchronized InputStream put(String url, String data, String login,
			String password) throws IOException {
		try {
